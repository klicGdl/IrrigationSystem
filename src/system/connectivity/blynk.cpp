...
void updateScreen(dev_conf_t t)
{
  ...
  Blynk.virtualWrite(V1, t.hora);
  Blynk.virtualWrite(V2, t.min);
  Blynk.virtualWrite(V3, t.duration);
  Blynk.virtualWrite(V5, t.lunes);  // if Monday is on, show all odd days on, otherwise all off
  Blynk.virtualWrite(V6, t.martes); // if Tuesday is on show all even days on, otherwise all off
}
...
BLYNK_WRITE(V3)
{
  temp_inf.duration = (param.asInt() * 60); // to convert to seconds
}
